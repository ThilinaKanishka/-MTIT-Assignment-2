...
This repository contains the Student Management microservice, Course Management microservice, Enrollment Management microservice, and an API Gateway for the University Management System domain. All services are written in Node.js/Express and ship with OpenAPI/Swagger UI for quick inspection and testing.
...
- `course-service/` — Node.js microservice for Course Management
  ...
  - `src/docs/swagger.js` — OpenAPI specification used by Swagger UI
- `enrollment-service/` — Node.js microservice for Enrollment Management
  - `src/index.js` — Express app entry point
  - `src/routes/enrollment_routes.js` — REST endpoints
  - `src/models/enrollment_model.js` — Sequelize ORM model
  - `src/database/db.js` — Database connection
  - `src/config.js` — Configuration
  - `src/data/enrollment.db` — SQLite database
...
## Install and run the enrollment service
...
The service starts on `http://localhost:3008` by default and uses SQLite for data persistence.
...
- Enrollments (enrollment-service)
  ...
  - `GET /api/enrollments/{id}` — Fetch a single enrollment
  - `POST /api/enrollments` — Create an enrollment (requires `student_id`, `course_id`)
...
- `/<service-path>/**` — Proxies traffic to downstream services. Defaults:
  ...
  - `/api/enrollments/**` → `http://localhost:3008` (Enrollment Service)
...
- Enrollment service docs: `http://localhost:3008/api-docs`
